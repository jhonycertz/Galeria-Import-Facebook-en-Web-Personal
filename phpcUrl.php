		...
		public function FacePageAlbum($id, $albumId, $aurl, $appId, $appSecret)
		{
			...
			if($id)
			{
				...
				if($albumId && $this->albumChk($albumId))
				{
					...
					echo '<a id="back" href="javascript:history.go(-1)">Atras</a>';
					echo '<a id="backAlbums" href="'.$this->PAGE.'">Ir a los albums</a>';
					echo '<br clear="all" />';
					...
					{
						echo "<a class='ImageLink' href = '".$v->source."'><img width='110px' height = '210px' src='".$v->source."' /></a>";
					}
					return true;
				}
				else if ($aurl)
				...
				else
				{
					...
					foreach($json->data as $v)
					{
					echo "<div class ='ImgWrapper' style= 'width:250px; height:300px; float:left; margin: 8 15 10 15;border: 5px solid white;'>";
					echo "<div style = 'width:250px; height:190px; overflow:hidden'>
							<img width='250px' src='https://graph.facebook.com/".$v->id."/picture' />
					      </div>";
						echo  $v->from->name."<br>";
						echo  "<a href = '".$this->PAGE;
						echo  "?";
						echo  "aid=".$v->id."'>".$v->name."</a>";
						echo  "<br style = 'width:250px; height:250px;'>Imagenes:".$v->count."<br>";
